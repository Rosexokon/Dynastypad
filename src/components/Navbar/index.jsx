  ...
  return (
    <div className="container">
      <div  className='w-full flex flex-col lg:flex-row items-center justify-between bg-[#FFFFFF] lg:p-[15px_30px]   lg:space-x-20 relative'>
        ...
        <div className={`${ isOpen ? 'left-0 z-50' : 'left-[-100%] lg:left-0'} fixed top-0 transition-all lg:relative flex flex-col lg:flex-row lg:justify-between items-start lg:items-center w-[70%] lg:w-full bg-gray-50 lg:bg-transparent h-screen lg:h-fit p-[30px_15px] lg:p-0`}>
          <div className='flex flex-col lg:flex-row items-start lg:items-center space-y-6 lg:space-y-0 lg:space-x-6'>
            {
              navLinks.map(({id,name, link})=> (
                <Link to={link} key={id} className='font-grotesk font-[500] text-[16px] text-[#04042A]'>{name}</Link>
              ))
            }
          </div>
          <div className='flex flex-col lg:flex-row items-center mt-[30px] lg:m-0 space-y-6 lg:space-y-0 lg:space-x-6'>
            <button className='px-3 py-2 font-[grotesk] border-[#4176FF] border-[1px] text-[#4176FF] bg-[#fff] btnwlt rounded-md'>Apply For Lisiting</button>
